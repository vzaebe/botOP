...
class EventService:
    ...
    async def register_user(self, user_id: int, event_id: str) -> Registration:
        ...
        if not event:
            raise ValidationError("Мероприятие не найдено")
        active_regs = await self.reg_repo.list_by_event(event_id)
    ...
    async def confirm_registration(self, user_id: int, event_id: str):
        reg = await self.reg_repo.get(user_id, event_id)
        if not reg:
            raise ValidationError("Регистрация не найдена")
        await self.reg_repo.update_status(reg.id, "confirmed")  # type: ignore
